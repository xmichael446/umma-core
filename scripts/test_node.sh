...
#
# Example:
# CHAIN_ID="umma-1" HOME_DIR="~/.uumad" TIMEOUT_COMMIT="500ms" CLEAN=true sh scripts/test_node.sh
# CHAIN_ID="local-2" HOME_DIR="~/.uumad" CLEAN=true RPC=36657 REST=2317 PROFF=6061 P2P=36656 GRPC=8090 GRPC_WEB=8091 ROSETTA=8081 TIMEOUT_COMMIT="500ms" sh scripts/test_node.sh
#
...
export HOME_DIR=$(eval echo "${HOME_DIR:-"~/.uumad"}")

export BINARY=${BINARY:-ummad}
...
ummad config keyring-backend $KEYRING
ummad config chain-id $CHAIN_ID
...
command -v ummad > /dev/null 2>&1 || { echo >&2 "ummad command not found. Ensure this is setup / properly installed in your GOPATH."; exit 1; }
...
from_scratch () {
  ...
  # GlobalFee
  update_test_genesis '.app_state["globalfee"]["params"]["minimum_gas_prices"]=[{"amount":"0.002500000000000000","denom":"stake"}]'
  ...
  BINARY add-genesis-account $KEY 10000000stake,1000utest --keyring-backend $KEYRING
  BINARY add-genesis-account feeacc 1000000stake,1000utest --keyring-backend $KEYRING

  BINARY gentx $KEY 1000000stake --keyring-backend $KEYRING --chain-id $CHAIN_ID
...
BINARY start --pruning=nothing  --minimum-gas-prices=0stake --rpc.laddr="tcp://0.0.0.0:$RPC"